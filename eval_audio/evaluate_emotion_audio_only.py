'''
This file uses Qwen2-7B-Instruct, rather than Qwen2-7B which is designed for autocompletion

Ablation variant of evaluate_graph_emotion_2.py: the prompt carries only the audio
and the bare task instruction. No emotion graph is ever read or embedded in the
prompt, so any accuracy difference against evaluate_graph_emotion_2.py isolates the
effect of the graph text itself.
'''
...
# The jsonl's own 'prompt' field (which embeds the emotion graph) is intentionally
# never read by this script -- only 'audio', 'source', and 'gt' are used.
ds_collections = {
...
# Each line is parsed into audio path, source, and ground truth label. Unlike
# evaluate_graph_emotion_2.py, the jsonl's 'prompt' field (graph + task text) is
# discarded -- every sample gets the same bare task instruction below instead.
class AudioDataset(torch.utils.data.Dataset):
    def __init__(self, ds):
        ...
        self.task = (
        "Given the audio file, complete the following task.\n"
    ...
    def __getitem__(self, idx):
        ...
        source = data['source']
        conversation = [
            {"role": "system", "content": "You are an expert audio analyst. You will be given an audio file. Your task is to identify the emotion, only output letters A-E. (A) Neutral (B) Happy (C) Sad (D) Surprised (E) Angry\n"
        "Answer ONLY with the option letter A, B, C, D, or E"},
            {"role": "user", "content": [
...
if __name__ == '__main__':
    ...
    if args.checkpoint == 'Qwen/Qwen2-Audio-7B-Instruct':
    ...
    results_file = f'/content/drive/MyDrive/MELD.Raw/{args.dataset}_audio_only_{time_prefix}.json'
